...
export function useFileLoading(
  ...
  const handleFileUpload = useCallback(
    async (files: File[]) => {
      ...
      if (nvdFiles.length > 0) {
        ...
        const surfaceExts = ['.mz3', '.obj', '.vtk', '.stl', '.gii', '.off', '.ply', '.surf'];
        const isSurface = (f: File) => surfaceExts.some(ext => f.name.toLowerCase().endsWith(ext));
        const surfaceFiles = files.filter(isSurface);
        const volumeFiles = files.filter(f => !isSurface(f));

        if (surfaceFiles.length > 0) {
          const meshOptionsList = surfaceFiles.map((file) => ({
          ...
          nv.updateGLVolume();
          updateSurfaceDetails();
          setActiveTab("surfaceDetails");
        }

        if (volumeFiles.length > 0) {
          const promises = volumeFiles.map(async (file) => {
    ...
    [nvRef, showUploader, currentImageIndex, loadNvdData, applyViewerOptions, incrementVolumeVersion, setShowUploader, setCurrentImageIndex, updateSurfaceDetails, setActiveTab],
  );
  ...
  const handleSurfaceFileSelect = useCallback(
    async (file: FileItem) => {
      ...
      try {
        ...
        if (!nv.canvas) throw new Error("Canvas failed to initialize");
        ...
        console.error("Error loading surface file:", error);
      }
    },
    [nvRef, showUploader, setShowUploader, updateSurfaceDetails],
  );
  ...
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const volParams = urlParams.getAll("vol");

    if (volParams.length > 0) {
      (async () => {
        let retries = 0;
        while (!nvRef.current?.canvas && retries < 50) {
          await new Promise((resolve) => setTimeout(resolve, 100));
          retries++;
        }
        if (!nvRef.current?.canvas) return;
        const nv = nvRef.current;
        setShowUploader(false);
        const volumes = volParams.map((volParam) => ({
          url: volParam,
          name: volParam.split("/").pop() || volParam,
        }));
        await nv.loadVolumes(volumes);
        applyViewerOptions();
        incrementVolumeVersion();
        if (nv.volumes.length > 0) setCurrentImageIndex(0);
      })();
    }
  }, []); // eslint-disable-line react-hooks/exhaustive-deps
