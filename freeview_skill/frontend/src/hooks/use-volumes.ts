  ...
  const handleEditVolume = useCallback(
    async (imageIndex: number) => {
      ...
      try {
        ...
        if (!loadSuccess) {
          console.error("Failed to load drawing - re-adding as volume");
          await nv.addVolume(drawingImage);
          incrementVolumeVersion();
          return;
        }

        originalVolumeRef.current = drawingImage;
        nv.setDrawOpacity(drawingOptions.opacity);
        nv.updateGLVolume();
        ...
        setDrawingOptions((prev) => ({
          ...
          enabled: true,
          mode: "none",
          filename: volumeName.replace(/(\.(nii|mgz|mgh|img|hdr))(\.gz)?$/i, "") + ".nii.gz",
