...
export default function Footer() {
  ...
  const labels = useFreeBrowseStore((s) => s.drawingOptions.labels);

  // Active drawing: label from draw bitmap value
  const drawBitmapLabel =
    locationData?.drawValue != null
      ? (labels.find((l) => l.value === locationData.drawValue)?.name ?? null)
      : null;

  // Saved annotation: for each volume, check if its integer value matches a label
  const labelForVolume = (value: number): string | null => {
    const intVal = Math.round(value);
    if (intVal <= 0 || intVal > 255) return null;
    return labels.find((l) => l.value === intVal)?.name ?? null;
  };
      ...
      {locationData ? (
        <div className="space-y-2">
          ...
          {locationData.voxels.map((vol, index) => {
            const labelName =
              (index === 0 ? drawBitmapLabel : null) ?? labelForVolume(vol.value);
            return (
              <div
                key={index}
                className={cn(
              ...
              >
                ...
                <div className="text-muted-foreground">
                  ...
                  {labelName && (
                    <span className="ml-2 text-foreground font-medium">{labelName}</span>
                  )}
                </div>
              </div>
            );
